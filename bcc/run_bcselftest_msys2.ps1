...
$BccDir = $PSScriptRoot
$ProjectRoot = (Resolve-Path (Join-Path $BccDir "..")).Path
...
try {
  ...
  if ($Clean) {
    Invoke-External -File $makeCmd.Source -Arguments @("-i", "-C", $BccDir, "clean") -StepName "Clean bcc target"
  }

  if (-not $SkipBuild) {
    Invoke-External -File $makeCmd.Source -Arguments @("-C", $BccDir, ("BC_EXT=" + $BcExt)) -StepName "Build bcc target"
  }

  if (-not $SkipTest) {
    $exeCandidates = @(
      (Join-Path $BccDir "bcc.exe"),
      (Join-Path $BccDir "bcc")
    )
