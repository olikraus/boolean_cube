...
$FcgiDir = $PSScriptRoot
$ProjectRoot = (Resolve-Path (Join-Path $FcgiDir "..")).Path
...
$LogFile = Join-Path $LogDir ("fcgi-build-" + $timestamp + ".log")
$LatestLog = Join-Path $LogDir "fcgi-latest.log"
...
try {
  Write-Log "boolean_cube fcgi build runner started"
  Write-Log ("ProjectRoot: " + $ProjectRoot)
  Write-Log ("FcgiDir: " + $FcgiDir)
  Write-Log ("MsysRoot: " + $MsysRoot)
  Write-Log ("BcExt: " + $BcExt)
  ...
  if ($Clean) {
    Invoke-External -File $makeCmd.Source -Arguments @("-i", "-C", $FcgiDir, "clean") -StepName "Clean fcgi target"
  }

  if (-not $SkipBuild) {
    Invoke-External -File $makeCmd.Source -Arguments @("-C", $FcgiDir, ("BC_EXT=" + $BcExt)) -StepName "Build fcgi target"
  }

  $exeCandidates = @(
    (Join-Path $FcgiDir "bcc.fcgi.exe"),
    (Join-Path $FcgiDir "bcc.fcgi")
  )
  ...
  if ($null -eq $fcgiExe) {
    throw "Could not find fcgi executable after build. Checked: $($exeCandidates -join ', ')"
  }

  Write-Log ("Build output: " + $fcgiExe)
